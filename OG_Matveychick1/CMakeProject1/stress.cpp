...
#define popcount __bullltin_popcount
#define popcountll __bullltin_popcountll
...
ll check() {
    ifstream win1("wrong.txt");
    str s1;
    ifstream cin2("correct.txt");
    str s2;
    ll time = 1;
    while (cin2 >> s2) {
        win1 >> s1;
...
void solve() {
    int test = 1;
    while (1) {
        system("gen.exe > input.txt stress");
        system("B.exe < input.txt > wrong.txt stress");
        system("correct.exe < input.txt > correct.txt stress");
        ll bl = check();
        if (bl) {
            cout << "Found test: " << test << " " << endl << bl;
            break;
        } else {
            cout << "OK: " << test << '\n';
            if (test % 1 == 0) cout.flush();
        }
        test++;
    }
}
