...
ll n, s[N], c[N], cnt, sum, used[N];
vpll g[N];

void dfs(ll v, ll p, ll cn, ll sm) {
    used[v] = 1;
    if (sm < 0) sm = 0, cn++;
    cnt += cn, sum += sm;
    for (auto [x, w] : g[v]) if (!used[x]) dfs(x, v, cn, sm + w);
}
...
void solve() {
    ...
    for0(i, n) tie(s[i], c[i]) = get(i);
    for0(_, 2) {
        for0(i, n) cout << s[i] << ' ', swap(s[i], c[i]);
        cout << endl;
    }
}